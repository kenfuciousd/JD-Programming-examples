...
def findMin(inString):
	...
	for game in gamesList:
		...
		#print("%s %s", greenTotal.group(0), type(greenTotal))
		if(greenType is not None):
			greenTotal = int(re.search('\d+', str(greenType.group(0))).group(0))
			if(greenTotal > greenMin):
				greenMin = greenTotal
		#print(greenTotal)
		redType = re.search('\d+ red', game)
		#print(type(redTotal), redTotal)
		if(redType is not None):
			redTotal = int(re.search('\d+', str(redType.group(0))).group(0))
			if(redTotal > redMin):
				redMin = redTotal
		#print(redTotal)
		blueType = re.search('\d+ blue', game)
		if(blueType is not None):
			blueTotal = int(re.search('\d+', str(blueType.group(0))).group(0))
			if(blueTotal > blueMin):
				blueMin = blueTotal
		print("Found Minimums for Red: " + str(redMin) + " - Blue: " + str(blueMin) + " - Green: " + str(greenMin))
		# this is a stupid hack to avoid multiplying by zero, just in case
		if (redMin == 0):
			redMin = 1
		if (greenMin == 0):
			greenMin = 1
	...
	mathTotal = redMin * greenMin * blueMin
	print(" !!!! and the running total is: " + str(mathTotal))
	return mathTotal
